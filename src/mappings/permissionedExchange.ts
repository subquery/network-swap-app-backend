...
function calculateTradeAmount(totalTradeAmount: bigint, event: AcalaEvmEvent<TradeEvent['args']>): bigint {
...
export async function handleExchangeOrderSent(event: AcalaEvmEvent<ExchangeOrderSentEvent['args']>): Promise<void> {
...
export async function handleTrade(event: AcalaEvmEvent<TradeEvent['args']>) {
  ...
  const permissionedExchange = PermissionedExchange__factory.connect(EXCHANGE_DIST_ADDRESS, new FrontierEthProvider());
...
export async function handleOrderSettled(event: AcalaEvmEvent<OrderSettledEvent['args']>) {
